...
void	ft_ld_help(t_process *p, char position)
{
	if (position == -48)
	{
		p->args[1] %= IDX_MOD;
		...
		{
			p->regs[p->args[2]] =
					(unsigned int)ft_take_ind((p->mem_addres - 5 + p->args[1])
	% IDX_MOD);
...
void	ft_ld(t_process *p)
{
	char	position;

	position = g_dt.map[0][ft_get_value(ft_get_value(p->mem_addres + 1))];
	ft_ld_help(p, position);
}
